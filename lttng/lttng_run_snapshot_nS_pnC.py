#!/usr/bin/python3
# snapshot n sessions p:n channels per session, where p is the maximum number of tracepoint providers. 
import os
...
if __name__=="__main__":
    ...
    index = 0;
    while (index < 25):
        lttng_cmd_str_1 = "lttng create tp_session_{0} --snapshot > /dev/null".format(str(index))
...
#        lttng_cmd_str_5 = "lttng enable-channel --num-subbuf 16 --subbuf-size 512k -u tp_channel_{0} -s tp_session_{1}".format(str(index), str(index % 25))
        lttng_cmd_str_5 = "lttng enable-channel -u tp_channel_{0} -s tp_session_{1}".format(str(index), str(index % 25))
